...
result_84 = pd.read_csv("/data/ephemeral/level2-klue-nlp-10/code/prediction/output_84.csv")  # 84
...
for idx in tqdm(range(len(gj))):
    ...
    num84_probs_list =  ast.literal_eval(result_84.loc[idx, 'probs'])
      #34 - 15% #36 - 17% #31 - 17% #47 - 17% #66 - 17% #84 - 17%
    ensemble_probs = [(0.15)*x + (0.15)*y + (0.16)*z + (0.18)*a + (0.18)*b + (0.18)*c for x, y, z, a, b, c in zip(gj_probs_list, 
                                                                                                gj2_probs_list, 
                                                                                                     jw_probs_list, 
                                                                                                     sota_probs_list, 
                                                                                                     cv_probs_list, 
...
final.to_csv("/data/ephemeral/level2-klue-nlp-10/code/prediction/output_ensemble6_0113_2.csv", index = False)